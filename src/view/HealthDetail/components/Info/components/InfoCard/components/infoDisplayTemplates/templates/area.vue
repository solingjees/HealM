...
export default {
  ...
  methods: {
    getEchartOptions () {
      ...
      return {
        // title:{
        //   text: moment(this.date).format('YYYY') === moment().format('YYYY')?
        //   moment(this.date).format('MM-DD') + '的'+this.healthItemAttr.name+'表' : this.date +'的'+this.healthItemAttr.name+'表'
        // },
        tooltip: {
          axisPointer: {
            type: 'line'
          ...
          formatter: params => {
            ...
            return moment(that.recordData[index].createTime).format('HH:ss') + '<br />' + tipsArr.reduce((prev, cur, index, arr) => {
